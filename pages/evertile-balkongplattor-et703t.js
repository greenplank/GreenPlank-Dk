import React from "react";
import Layout from "../components/layout";
import HeaderTwo from "../components/header/header-two";
import StickyHeader from "../components/header/sticky-header";
import PageHeader from "../components/page-header";
import { Container, Row, Col } from "react-bootstrap";
import Grid from '@material-ui/core/Grid';
import Image1 from "../assets/images/products/innerPages/golvplattor/Untitled-1.webp";

import Imagecolor1 from "../assets/images/colors/evertile-balkongplattorGp703t/Balcony-Tile-Evertile-Beach-Grey.png";
import Imagecolor2 from "../assets/images/colors/evertile-balkongplattorGp703t/Balcony-Tile-Evertile-Charcoal.png";
import Imagecolor3 from "../assets/images/colors/evertile-balkongplattorGp703t/Balcony-Tile-Evertile-Composite-Teak.png";

import Head from "next/head";
import Imagesurface from "../assets/images/surface/Asset-83-8.png";

import pdfimg1 from "../assets/images/projects/greenplank-brochure-title-1.jpg";
import pdfimg2 from "../assets/images/projects/pic.png";
import pdfimg3 from "../assets/images/projects/pic1.png";
import pdfimg4 from "../assets/images/projects/swedish.png";
import pdfimg6 from "../assets/images/projects/greenplank-composite-decking-installation-guide-1.jpg";

import Footer from "../components/footer";

const evertilebalkongplattoret703t = () => {
    return (
        <>
            <Head>
                <title>EVERTILE™-ET703T</title>
                <meta name="title" content="Plattor | EVERTILE™ Balkongplattor-ET703T - GreenPlank.se" />
                <meta name="description" content="ALLA komposittrall och balkongplattor är det mest populära alternativet för att underhålla plattor i form av överkomliga priser." />
            </Head>
            <Layout pageTitle="Evertile-et703t">
                <HeaderTwo />
                <StickyHeader />
                <PageHeader title="Evertile-703t" crumbTitle="evertile-et703t" />
                <section className="blog-details pt-40 pb-40">
                    <Container>
                        <Row>
                            <Col md={12} lg={12}>
                                <div className="page-wrapper pt-60 pb-60">
                                    {/*Smart Classic section start*/}
                                    <div className="project-section">
                                        <div className="container">
                                            <div className="row">

                                                {/*====================  Star Golvplattor Section-1 content ====================*/}
                                                <div className="col-lg-12 col-12 mb-80 pl-30 pl-sm-15 pl-xs-15">
                                                    <div className="project-details">
                                                        <h2>EVERTILE ™ Gårdhave og Altanfliser</h2>
                                                        <strong>Overkommelige kompositfliser til din stil og dit budget – PVC-fri!</strong>
                                                        <div class="et_pb_text_inner">
                                                            <div class="ce_text block">

                                                                <strong>Inspireret af den skandinaviske designtradition, der er kendetegnet ved æstetik, enkelhed og funktionalitet.

                                                                </strong>
                                                                <p>
                                                                EVERTILE ™ kompositdæk og altan

                                                                </p>
                                                                <p>
                                                                EVERTIEL ™ gårdhave og balkon er de mest populære dækfliser med lav vedligeholdelse, når det kommer til overkommelige priser. Lavet af kompositter af naturlige fibre (træ og genanvendt HDPE-plast) og med en attraktiv skridsikker overflade, der eliminerer rådne, algevækst og misfarvning. Meget glat, elegant, ribbet / rillet, børstet overflade muliggør god trækkraft i våde områder, f.eks. I nærheden af ​​bassiner, de kan gå barfodet.

                                                                </p>
                                                                <p>
                                                                EVERTILE ™ terrasse og balkonfliser giver hver flad overflade et nyt, usædvanligt look. Vores fliser har en skridsikker og barfodsvenlig overflade. Forbindelsesstropper er integreret i hver dækplade på alle fire sider, hvilket sikrer en nøjagtig, automatisk justering af pladen og låser hver dækplade præcist og sikkert uden stor anstrengelse.

                                                                </p>
                                                                <div>
                                                                    <ul>
                                                                        <li><strong>Føj stil til enhver størrelse plads</strong> – INSTACLIK ™ terrasse og balkon fliser kan let installeres og fjernes. Der er et personligt design touch, der bevæger sig, når du gør dette.</li>
                                                                        <li><strong>Vælg rummet, tilføj fliser, nyd  </strong>– de fleste brugere har installeret INSTACLIK ™ terrasse og balkonfliser på maksimalt 90 minutter. INSTACLIK ™ terrasse- og altanfliser kan bruges på enhver solid, plan udvendig overflade.</li>
                                                                        <li><strong>Smukt på begge sider </strong>– gårdhave og altanfliser ser smukke ud, men hver flise er bygget på solid teknologi – en slidstærk, indviklet flisestøtte, der giver god luftcirkulation og dræning og giver hurtigere tørretid for den hårde overflade nedenunder.</li>
                                                                        <li><strong>Udviklet til at modstå</strong>– Et udendørs område skal være så holdbart, som det er smukt – det er derfor, INSTACLIK ™ loft- og altanfliser kommer med en begrænset garanti på 5 år.</li>
                                                                    </ul>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                                {/*====================  End of Golvplattor Section-1 content  ====================*/}

                                                {/*====================  Star Golvplattor Section-2 content  ====================*/}
                                                <div className="col-lg-12 mb-80">
                                                    <div className="project-gallery-wrapper">
                                                        <div className="row">
                                                            <div className="col-md-6 col-mobile-6 mb-20">
                                                                <div className="single-gallery-project">
                                                                    <div className="single-gallery-project__image">
                                                                        <img src={Image1} className="img-fluid" alt="" />
                                                                    </div>
                                                                </div>
                                                            </div>
                                                            <div className="col-md-6 col-mobile-6 mb-20">
                                                                <div className="project-information">
                                                                    <h3>Produkt information</h3>
                                                                    <table width="100%">
                                                                        <tbody>
                                                                            <tr>
                                                                                <td align="left"><strong>Overflade:</strong></td>
                                                                                <td align="left">Skridsikrede riller, autentiske
                                                                                    og naturlige træfarver.
                                                                                </td>
                                                                            </tr>
                                                                            <tr>
                                                                                <td align="left"><strong>Størrelse:</strong></td>
                                                                                <td align="left">ca. 295 * 295 mm</td>
                                                                            </tr>
                                                                            <tr>
                                                                                <td align="left"><strong>Højde:</strong></td>
                                                                                <td align="left">ca. 26 mm</td>
                                                                            </tr>
                                                                            <tr>
                                                                                <td align="left"><strong>Forbrug:</strong></td>
                                                                                <td align="left">ca. 11 stk til 1 m²
</td>
                                                                            </tr>
                                                                            <tr>
                                                                                <td align="left"><strong>Vægt:</strong></td>
                                                                                <td align="left">ca. 1,35 kg / tallerken
</td>
                                                                            </tr>
                                                                        </tbody>
                                                                    </table>
                                                                </div>

                                                                <div className="row mt-20">

                                                                    <Grid container spacing={3}>
                                                                        <Grid item xs={6} sm={4} lg={6}>
                                                                            <a href="/bestall-prov" target="_blank" className="thm-btn" >beställ prov</a>
                                                                        </Grid>
                                                                    </Grid>
                                                                </div>
                                                            </div>

                                                        </div>
                                                    </div>
                                                </div>
                                                {/*====================  End of Golvplattor Section-2 content  ====================*/}

                                                {/*====================  Star Golvplattor Section-3 content  ====================*/}
                                                <div className="col-12 mb-80">
                                                    <h3>VIS FARVETRENDER - indgroet med en naturlig og særpræg
</h3>
                                                    <hr />
                                                    <Grid container spacing={3}>
                                                        <Grid item xs={6} sm={2}>
                                                            <div className="single-gallery-project__image">
                                                                <img src={Imagecolor1} className="img-fluid" alt="" />
                                                                <h4 className="mt-20 text-center">Beach Grey</h4>
                                                            </div>
                                                        </Grid>
                                                        <Grid item xs={6} sm={2}>
                                                            <div className="single-gallery-project__image">
                                                                <img src={Imagecolor2} className="img-fluid" alt="" />
                                                                <h4 className="mt-20 text-center">Charcoal</h4>
                                                            </div>
                                                        </Grid>
                                                        <Grid item xs={6} sm={2}>
                                                            <div className="single-gallery-project__image">
                                                                <img src={Imagecolor3} className="img-fluid" alt="" />
                                                                <h4 className="mt-20 text-center">Teak</h4>
                                                            </div>
                                                        </Grid>
                                                    </Grid>
                                                </div>
                                                {/*====================  End of Golvplattor Section-3 content  ====================*/}

                                                {/*====================  Star Golvplattor Section-4 content ====================*/}
                                                <div className="col-lg-12 mb-80">
                                                    <div className="project-gallery-wrapper">
                                                        <div className="row">
                                                            <div className="col-md-6 col-mobile-6 mb-40 mt-20 pl-30 pl-sm-15 pl-xs-15">
                                                                <div className="project-details">
                                                                    <h2 className="mb-40 mt-20"><strong>produkt information</strong></h2>
                                                                    <h3>Farvevalg</h3>
                                                                    <p>
                                                                        Studiefotografering og / eller din browserenhed giver muligvis ikke en nøjagtig gengivelse af farverne. For at se farverne personligt, bedes du anmode om gratis prøver.
                                                                    </p>
                                                                    <h3>Naturlige skønhed</h3>
                                                                    <p>
                                                                        Vores produkter er designet til at ligne naturligt træ og kan have svag farve / struktur og skygge variationer fra plade til plade. Farvevariationen vil på ingen måde påvirke produktets ydeevne.</p>
                                                                    <h3>Oprethold skønheden</h3>
                                                                    <p>
                                                                        Alle udvendige overflader opsamler snavs, støv og forurenende stoffer og skal rengøres. Det er nødvendigt at rengøre dit kompositdæk med jævne mellemrum for at holde det så smukt som den dag, du installerede det.</p>
                                                                </div>
                                                            </div>
                                                            <div className="col-md-6 col-mobile-6 mb-40 mt-20 pl-30 pl-sm-15 pl-xs-15">
                                                                <div className="project-details">
                                                                    <h2 className="mb-40 mt-20"><strong>Pålidelig holdbarhed, overlegen kvalitet</strong></h2>
                                                                    <ul>
                                                                        <li><strong>1. </strong>Træfarvede farver skaber et autentisk kornmønster, der inviterer dig til at slappe af og nyde dit personlige tropiske paradis.</li>
                                                                        <li><strong>2. </strong>Der kræves ingen årlig farvning, forsegling eller maling</li>
                                                                        <li><strong>3. </strong>Ekstraordinær modstandsdygtighed over for UV-lys og farvetone</li>
                                                                        <li><strong>4. </strong>Høj skridsikkerhed giver en sikker overflade, selv i våd tilstand</li>
                                                                        <li><strong>5. </strong>Splinter ikke, splittes, rådner eller vrides</li>
                                                                        <li><strong>6. </strong>Fri for giftige tilsætningsstoffer</li>
                                                                        <li><strong>7. </strong>Let at installere … ingen specialværktøjer, fastgørelser eller klæbemidler</li>
                                                                        <li><strong>8. </strong>Indbyggede sammenlåsende faner på alle 4 sider</li>
                                                                        <li><strong>9. </strong>Bundfastgørelse: skruer i rustfrit stål</li>
                                                                        <li><strong>10. </strong>Funktionsdygtigt temperaturområde: -40 ° C til 60 ° C</li>
                                                                        <li><strong>11. </strong>Velegnet til indendørs og udendørs applikationer.</li>
                                                                    </ul>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                                {/*====================  End of Golvplattor Section-4 content  ====================*/}



                                                {/*====================  Star Golvplattor Section-6 content  ====================*/}
                                                <div className="col-12 mb-30 mt-20" >
                                                    <h3>
                                                        Download Pdfs
                                                    </h3>
                                                    <hr />

                                                    <Grid container spacing={3}>
                                                        <Grid item xs={6} sm={2}>
                                                            <a target="_blank" href="/pdfs/greenplank-natural-fibers-compsoite-products-brochure-2020-english.pdf">
                                                                <img src={pdfimg1} className="img-fluid" alt="" />
                                                            </a>
                                                        </Grid>
                                                        <Grid item xs={6} sm={2}>
                                                            <a target="_blank" href="/pdfs/Green-Plank-Composite-Decking-hollow-boards-Installation-Guide-2019%20(1).pdf">
                                                                <img src={pdfimg6} className="img-fluid" alt="" />
                                                            </a>
                                                        </Grid>
                                                        <Grid item xs={6} sm={2}>

                                                        </Grid>

                                                    </Grid>
                                                </div>
                                                {/*====================  End of Golvplattor Section-6 content  ====================*/}


                                            </div>
                                        </div>
                                    </div>
                                    {/*Projects section end*/}
                                </div>
                            </Col>
                        </Row>
                    </Container>
                </section>
                <Footer />
            </Layout>
        </>
    );
};

export default evertilebalkongplattoret703t;
